...
export class Bookmarks {
  ...
  public loadFrom(jsonObject, relativePath?) {
    ...
    if (relativePath) {
      for (let element of this.bookmarks) {
        element.fsPath = element.fsPath.replace(
          '$ROOTPATH$',
  ...
  public zip(relativePath?: boolean): Bookmarks {
    ...
    newBookmarks.bookmarks = JSON.parse(JSON.stringify(this.bookmarks)).filter(
      isNotEmpty
    );
    ...
    for (let element of newBookmarks.bookmarks) {
      element.fsPath = element.fsPath.replace(
        vscode.workspace.getWorkspaceFolder(vscode.Uri.file(element.fsPath)).uri
          .fsPath,
        '$ROOTPATH$'
      );
    }
