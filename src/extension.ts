...
export function activate(context: vscode.ExtensionContext) {
  ...
  function updateBookmarkSvg() {
    ...
    const content = fs.readFileSync(
      context.asAbsolutePath('images/bookmark.svg'),
      'utf8'
    );
    ...
    for (let i = 0; i <= 9; i++) {
      ...
      try {
        fs.writeFileSync(
          context.asAbsolutePath(`images/bookmark${i}-${v}.svg`),
          svgContent,
          'utf8'
        );
      } catch (err) {
  ...
  function updateBookmarkDecorationType() {
    const backgroundLineColor: string = vscode.workspace
      .getConfiguration('numberedBookmarks')
      .get('backgroundLineColor', '');
    const v = getCurrentSvgVersion();
    ...
    for (let index = 0; index < MAX_BOOKMARKS; index++) {
      ...
      const gutterIconPath: string = context.asAbsolutePath(
        `images/bookmark${index}-${v}.svg`
      );
  ...
  function updateDecorations() {
    ...
    if (
      ...
      activeEditor.document.lineAt(0).text === ''
    ) {
  ...
  vscode.commands.registerCommand('numberedBookmarks.list', () => {
    ...
    for (let element of activeBookmark.bookmarks) {
      ...
      if (
        element !== -1 &&
        element <= vscode.window.activeTextEditor.document.lineCount
      ) {
        let lineText = vscode.window.activeTextEditor.document.lineAt(element)
          .text;
        element++;
  ...
  vscode.commands.registerCommand('numberedBookmarks.listFromAllFiles', () => {
    ...
    for (let index = 0; index < bookmarks.bookmarks.length; index++) {
      totalBookmarkCount =
        totalBookmarkCount + bookmarks.bookmarks[index].bookmarks.length;
    }
    ...
    // push the items
    let items: vscode.QuickPickItem[] = [];
    let activeTextEditorPath = vscode.window.activeTextEditor
      ? vscode.window.activeTextEditor.document.uri.fsPath
      : '';
    let promisses = [];
    let currentLine: number = vscode.window.activeTextEditor
      ? vscode.window.activeTextEditor.selection.active.line + 1
      : -1;

    let currentWorkspaceFolder: vscode.WorkspaceFolder;
    if (activeTextEditorPath) {
      currentWorkspaceFolder = vscode.workspace.getWorkspaceFolder(
    ...
    Promise.all(promisses).then(values => {
      ...
      for (let index = 0; index < values.length; index++) {
        ...
        for (let indexInside = 0; indexInside < element.length; indexInside++) {
          ...
          if (
            elementInside.detail.toString().toLowerCase() ===
            activeTextEditorPath.toLowerCase()
          ) {
            items.push({
            ...
            let itemPath = removeBasePathFrom(
              elementInside.detail,
      ...
      itemsSorted = items.sort(function(
        ...
        if (
          a.detail.toString().indexOf('$(file-submodule) ') === 0 &&
          b.detail.toString().indexOf('$(file-directory) ') === 0
        ) {
          return -1;
        }

        if (
          a.detail.toString().indexOf('$(file-directory) ') === 0 &&
          b.detail.toString().indexOf('$(file-submodule) ') === 0
        ) {
          return 1;
        }
        ...
        if (
          a.detail.toString().indexOf('$(file-submodule) ') === 0 &&
          b.detail.toString().indexOf('$(file-submodule) ') === -1
        ) {
          return 1;
        }

        if (
          a.detail.toString().indexOf('$(file-submodule) ') === -1 &&
          b.detail.toString().indexOf('$(file-submodule) ') === 0
        ) {
          return -1;
        }
        ...
        if (
          a.detail.toString().indexOf('$(file-directory) ') === 0 &&
          b.detail.toString().indexOf('$(file-directory) ') === -1
        ) {
          return 1;
        }

        if (
          a.detail.toString().indexOf('$(file-directory) ') === -1 &&
          b.detail.toString().indexOf('$(file-directory) ') === 0
        ) {
          return -1;
        }
        ...
        onDidSelectItem: item => {
          ...
          if (!itemT.detail) {
            ...
            if (itemT.detail.toString().indexOf('$(file-directory) ') === 0) {
              filePath = itemT.detail
                .toString()
                .split('$(file-directory) ')
                .pop();
            } else {
              // with octicon - documento from other workspaceFolder
              if (itemT.detail.toString().indexOf('$(file-submodule)') === 0) {
                filePath = itemT.detail
                  .toString()
                  .split('$(file-submodule) ')
                  .pop();
                ...
                if (currentWorkspaceFolder) {
                  filePath =
                    currentWorkspaceFolder.uri.fsPath + itemT.detail.toString();
                } else {
                  if (vscode.workspace.workspaceFolders) {
                    filePath =
                      vscode.workspace.workspaceFolders[0].uri.fsPath +
                      itemT.detail.toString();
                  } else {
          ...
          if (
            ...
            vscode.workspace.openTextDocument(uriDocument).then(doc => {
              ...
              vscode.window
                .showTextDocument(doc, { preserveFocus: true, preview: true })
                .then(editor => {
                  revealLine(parseInt(itemT.label, 10) - 1);
                });
            });
      ...
      vscode.window.showQuickPick(itemsSorted, options).then(selection => {
        ...
        if (!selection.detail) {
          ...
          if (selection.detail.toString().indexOf('$(file-directory) ') === 0) {
            ...
            // no octicon - document inside project
            if (
              selection.detail.toString().indexOf('$(file-submodule)') === 0
            ) {
              newPath = selection.detail
                .toString()
                .split('$(file-submodule) ')
  ...
  function canSaveBookmarksInProject(): boolean {
    let saveBookmarksInProject: boolean = vscode.workspace
      .getConfiguration('numberedBookmarks')
  ...
  function loadWorkspaceState(): boolean {
    ...
    if (saveBookmarksInProject) {
      ...
      let bookmarksFileInProject: string = path.join(
        vscode.workspace.workspaceFolders[0].uri.fsPath,
        '.vscode',
      ...
      try {
        bookmarks.loadFrom(
          JSON.parse(fs.readFileSync(bookmarksFileInProject).toString()),
          true
        );
        return true;
      } catch (error) {
        vscode.window.showErrorMessage(
          'Error loading Numbered Bookmarks: ' + error.toString()
        );
        return false;
      }
    } else {
  ...
  function saveWorkspaceState(): void {
    ...
    if (saveBookmarksInProject) {
      ...
      fs.writeFileSync(
        bookmarksFileInProject,
        JSON.stringify(bookmarks.zip(true), null, '\t')
      );
    } else {
      context.workspaceState.update(
        'numberedBookmarks',
        JSON.stringify(bookmarks.zip())
      );
    }
  }
  ...
  function toggleBookmark(n: number, line: number) {
    ...
    if (!activeBookmark) {
      ...
      activeBookmark = bookmarks.fromUri(
        vscode.window.activeTextEditor.document.uri.fsPath
      );
    }
    ...
    if (index !== n) {
      ...
      let navigateThroughAllFiles: string = vscode.workspace
        .getConfiguration('numberedBookmarks')
        .get('navigateThroughAllFiles', 'false');
    ...
    switch (navigateThroughAllFiles) {
      case 'replace':
        ...
        if (activeBookmark.bookmarks[n] < 0) {
          ...
          for (let element of bookmarks.bookmarks) {
            if (
              element.fsPath !== activeBookmark.fsPath &&
              element.bookmarks[n] !== NO_BOOKMARK_DEFINED
            ) {
              // open and novigate
              ...
              vscode.workspace.openTextDocument(uriDocument).then(doc => {
                vscode.window
                  .showTextDocument(doc, undefined, false)
                  .then(editor => {
      ...
      case 'allowDuplicates':
        ...
        for (
          ...
          index < bookmarks.bookmarks.length;
          index++
        ) {
          let element = bookmarks.bookmarks[index];
          if (
      ...
      if (inWorkspace === currentWorkspaceFolder) {
        ...
        if (
          ...
          return (
            '$(file-submodule) ' +
            inWorkspace.name +
            /*path.sep + */ aPath.split(inWorkspace.uri.fsPath).pop()
          );
        }
      }
