...
export class Sticky {
  public static stickyBookmarks(
    event: vscode.TextDocumentChangeEvent,
    activeEditorCountLine: number,
    ...
    if (this.hadOnlyOneValidContentChange(event)) {
      ...
      if (event.document.lineCount !== activeEditorCountLine) {
        if (event.document.lineCount > activeEditorCountLine) {
          ...
          if (event.contentChanges[0].range.end.line - event.contentChanges[0].range.start.line === 1) {
            if (
              ...
              // the bookmarked one
              let idxbk = activeBookmark.bookmarks.indexOf(
          ...
          if (event.contentChanges[0].range.end.line - event.contentChanges[0].range.start.line > 1) {
            for (
              let i = event.contentChanges[0].range.start.line /* + 1*/;
              i <= event.contentChanges[0].range.end.line;
              i++
            ) {
        ...
        for (let index = 0; index < activeBookmark.bookmarks.length; index++) {
          ...
          if (eventcharacter > 0) {
            ...
            textInEventLine = textInEventLine
              .replace(/\t/g, '')
              .replace(/\s/g, '');
            if (textInEventLine === '') {
              eventcharacter = 0;
          ...
          // also =
          if (
            (activeBookmark.bookmarks[index] > eventLine &&
              eventcharacter > 0) ||
            (activeBookmark.bookmarks[index] >= eventLine &&
              eventcharacter === 0)
          ) {
      ...
      if (!updatedBookmark && event.contentChanges[0].text.length > 1) {
        ...
        if (
          ...
          vscode.window.activeTextEditor.document.lineAt(selection.end).range
            .end.character
        ) {
          lineMax--;
        }
      ...
      if (activeEditor.selections.length === 1) {
        if (event.contentChanges[0].text === '') {
          updatedBookmark = this.moveStickyBookmarks(
            'down',
            activeBookmark,
            activeEditor
          );
  ...
  private static moveStickyBookmarks(
    direction: string,
    activeBookmark: Bookmark,
    activeEditor: vscode.TextEditor
  ): boolean {
    ...
    if (selection.end.character === 0 && !selection.isSingleLine) {
      ...
      let posMin = new vscode.Position(
        selection.start.line + 1,
        selection.start.character
      );
      let posMax = new vscode.Position(
        selection.end.line,
        lineAt.range.end.character
      );
  ...
  private static hadOnlyOneValidContentChange(
    event: vscode.TextDocumentChangeEvent
  ): boolean {
    // not valid
    ...
    if (event.contentChanges.length === 1) {
      ...
      // has 2, but is it a trimAutoWhitespace issue?
      if (event.contentChanges.length === 2) {
        let trimAutoWhitespace: boolean = vscode.workspace
          .getConfiguration('editor')
          .get('trimAutoWhitespace', true);
        ...
        let secondRangeEmpty: boolean =
          event.contentChanges[1].text === '' &&
          event.contentChanges[1].range.start.line ===
            event.contentChanges[1].range.end.line &&
